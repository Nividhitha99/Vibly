...
exports.analyzeItem = async (item, type) => {
  try {
    ...
    const prompt = `Analyze this ${type} deeply to understand what it reveals about someone's personality, psychological traits, and emotional needs.
...
exports.combineAnalyses = async (itemAnalyses, movies, music, shows) => {
  try {
    ...
    const prompt = `Based on deep individual analysis of ${itemAnalyses.length} entertainment items, create a comprehensive personality profile.
...
4. Provides insights into relationship compatibility
5. Describes their entertainment taste profile
